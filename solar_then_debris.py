...
class BotPlayer(Player):
    ...
    def play_turn(self, rc: RobotController):
        if rc.get_turn() < 1500:
          ...
          if rc.can_build_tower(TowerType.SOLAR_FARM, 0, 1):
            rc.build_tower(TowerType.SOLAR_FARM, 0, 1)
          
        if rc.get_turn() == 1500:
          rc.sell_tower(self.find_tower(rc, 0, 0).id)
          rc.sell_tower(self.find_tower(rc, 0, 1).id)
